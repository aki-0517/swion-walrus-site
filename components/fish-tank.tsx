...
export default function FishTank({ walletAddress, isOwner }: FishTankProps) {
  ...
  const fetchWaterTankSBT = useCallback(async () => {
    ...
    try {
      ...
      // Add suiClient validation
      if (!suiClient) {
        console.error("suiClient is not initialized")
        throw new Error("SUI client is not initialized")
      }

      // Add NEXT_PUBLIC_PACKAGE_ID validation
      const packageId = process.env.NEXT_PUBLIC_PACKAGE_ID
      if (!packageId) {
        console.error("NEXT_PUBLIC_PACKAGE_ID is not defined")
      ...
      try {
        ...
        if (waterTank) {
          ...
          setIsLocalMode(false) // Default is blockchain mode
        } else {
      ...
      console.error("Error in fetchWaterTankSBT:", error)
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to fetch water tank",
        variant: "destructive",
      })
      setIsLocalMode(true)
    } finally {
  ...
  // Local mode save handling
  const handleLocalSave = () => {
    setIsLoading(true)
    
    // Simple local save only
    setTimeout(() => {
      setHasChanges(false)
      setIsLoading(false)
  ...
  // New implementation: Attach and save function
  const handleAttachAndSave = async () => {
    // For local mode, use simple save only
    if (isLocalMode) {
      handleLocalSave()
      return
    }
    ...
    try {
      ...
      if (validNFTs.length === 0) {
        ...
        toast({
          title: "Warning",
          description: "No valid Sui objects found. Please place NFT objects.",
          variant: "destructive",
        })
        setIsLoading(false)
        return
  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto">
      <div className="pixel-container p-4">
        ...
        <div
          className={`fish-tank w-full h-[600px] relative rank-${tankRank}`}
          style={{
